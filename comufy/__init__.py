...
class Comufy(object):
    ...
    def __init__(self, username='', password='' ):
        ...
        self.passoword = os.environ.get('COMUFY_PASSWORD', password)

comufy = Comufy()

def convert_from_comufy_timestamp(value):
    """
...
def send_api_call(data, add_access_token=True):
    ...
    if add_access_token:
        if not get_access_token():
            return False, None
        data['token'] = comufy.access_token
...
def authenticate_app():
    ...
    data = {
        u'cd':131,
        u'user':comufy.username,
        u'password':comufy.password
    }
    
    #Step X: Attempt to authenticate the user against Comufy's API
    success, message = send_api_call( data, add_access_token=False )
    ...
    if success:
        ...
        if message.get(u'cd') == 235:
            comufy.access_token = message.get(u'tokenInfo').get(u'token')
            comufy.expiry_time = convert_from_comufy_timestamp( message.get(u'tokenInfo').get(u'expiryTime') )
            return True
        else:
            ...
            comufy.access_token = None
            comufy.expiry_time = None
            log.debug(
                """
...
def has_token_expired():
    ...
    global comufy
    if comufy.expiry_time is not None and comufy.expiry_time > datetime.now():
        return False
...
def get_application_tags(application_name):
    """
...
def add_application_user(application_name, user_details, add_new_tags=False):
    """
    ...
    current_tags = get_application_tags()
...
def add_application_users( application_name, users_details ):
    """
...
def get_application_users(application_name):
    """
    """
    data = {
        u'cd':82,
...
def send_message(application_name, description, content, fb_ids,
...
#TAG STUFF
def register_facebook_application_tag(application_name, tags):
    """
...
#TAG STUFF
def unregister_facebook_application_tag(application_name, tag):
    """
